...
test_that("visits() returns appropriate, non-empty dataframe", {
  ...
  for (r in list(finches, finches_lg, chickadees)) {
    ...
    expect_equal(
      v$date,
      lubridate::as_date(v$start, tz = lubridate::tz(v$start))
    )
    expect_true(all(as.numeric(v$end - v$start) >= 0))
  }
})
...
test_that("visits() jumps over obs of diff animals at diff loggers", {
  ...
  for (r in list(finches, finches_lg, chickadees)) {
    v <- visits(r, bw = 10000000000000000000) %>%
      dplyr::arrange(start) %>%
      dplyr::mutate(
        okay = (animal_id != dplyr::lead(animal_id)) |
          (animal_id == dplyr::lead(animal_id) &
            logger_id != dplyr::lead(logger_id))
      )
  ...
  for (q in c("finches", "finches_lg")) {
    r <- get(q)
    if (q == "chickadees") {
      r <- dplyr::filter(r, experiment == "exp2")
    }
        ...
        if (
          ...
          error <- rbind(
            ...
            data.frame(
              data = q,
              type = "logger",
              visits = v$visits[v$logger_id == l][i],
              id = l
            )
          )
        }
  ...
  expect_true(
    nrow(error) == 0,
    info = "At least one animal_id or logger_id has overlapping visits"
  )
})
...
test_that("visits() with group_by", {
  coords <- read.csv(system.file(
    "extdata",
    "chickadees_logger_index.csv",
    package = "feedr"
  )) %>%
    dplyr::mutate(
      logger_id = paste0(experiment, "-", logger_name),
  ...
  expect_silent(
    dplyr::group_by(c, experiment) %>%
      dplyr::do(visits(., bw = 3, allow_imp = TRUE))
  )
})
