test_that("map_prep", {
  ...
  expect_error(
    map_prep(
      p = dplyr::select(p, -logger_id),
      m = dplyr::select(m, -logger_id),
      summary = "sum"
    ),
...
test_that("Custom summaries", {
  ...
  m <- move(visits(finches)) %>%
    ...
    dplyr::summarize(
      path_use = length(move_path) / logger_n[1],
      .groups = "drop"
    )
  ...
  # Catch problems
  expect_error(
    map_prep(p = p, m = m_error, summary = "none"),
    "Movement data should have identical path_use"
  )
  expect_error(
    map_prep(p = p_error, m = m, summary = "none"),
    "Presence data should be summarized to have one row"
  )
  expect_error(
    map_prep(p = p_error2, m = m, summary = "none"),
    "Movements \\(m\\) and presence \\(p\\) data do not both have"
  )
  expect_error(
    map_prep(p = p_error2, m = m_indiv, summary = "none"),
