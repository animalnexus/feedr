test_that("move() returns appropriate, non-empty dataframe", {
  ...
  expect_match(
    names(m)[1:3],
    "^animal_id$|^date$|^time$|^logger_id$|^direction$|^move_dir$|^move_path$|^strength$"
  )
...
test_that("move() returns non-impossible data", {
  ...
  expect_true(all(
    m$time[m$direction == "arrived"] >= m$time[m$direction == "left"]
  ))
  expect_true(all(
    m$logger_id[m$direction == "arrived"] != m$logger_id[m$direction == "left"]
  ))
})
...
test_that("move() returns expected data", {
  ...
  expect_equal(
    m$logger_id[1:2],
    factor(c("2400", "2100"), levels = c("2100", "2200", "2400", "2700"))
  )
  expect_equal(
    m$time[1:2],
    as.POSIXct(
      c("2016-01-29 08:35:26", "2016-01-29 11:21:23"),
      tz = "Etc/GMT+8"
    )
  )
...
test_that("move() handles zero movements and single animal", {
  ...
  expect_silent(
    m <- visits(finches) %>% dplyr::filter(animal_id == "062000043E") %>% move()
  )
  expect_s3_class(m, "data.frame")
  ...
  expect_silent(
    m <- visits(finches) %>%
      dplyr::filter(animal_id == "062000043E") %>%
      move(., all = TRUE)
  )
  expect_s3_class(m, "data.frame")
  expect_length(m, 17)
