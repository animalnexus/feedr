  ...
  expect_equal(
    summaries(p),
    {
      dplyr::group_by(p, .data$logger_id, .data$lat, .data$lon) %>%
        dplyr::summarise(
          amount = sum(.data$length) / .data$animal_n[1],
          .groups = "drop"
        )
    },
    ignore_attr = TRUE
  ...
  expect_equal(
    summaries(m),
    {
      dplyr::group_by(
        ...
        dplyr::summarise(
          path_use = length(.data$move_path) / .data$animal_n[1],
          .groups = "drop"
        )
    },
    ignore_attr = TRUE
