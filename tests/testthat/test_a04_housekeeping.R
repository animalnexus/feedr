...
test_that("check_ids catches and removes correctly", {
  ...
  check_ids(finches, b) %>%
    expect_message(
      "Some animal_ids present in your data do not exist in the animal_id index: 0620000514, 041868D861, 062000043E"
    ) %>%
    suppressMessages()
  ...
  expect_message(
    f <- check_ids(r, b),
    paste0(
      "Some animal_ids present in your animal_id index, ",
  ...
  expect_error(
    check_ids(r, b),
    "You have some ids in your read data that are not 10 characters long"
  )
  expect_error(
    check_ids(r, b, id_length = 4),
