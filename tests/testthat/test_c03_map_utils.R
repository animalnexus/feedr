test_that("get_locs handles custom data types", {
  p <- presence(visits(finches)) %>%
    ...
    dplyr::summarize(
      amount = sum(.data$length) / .data$logger_n[1],
      .groups = "drop"
    )
  ...
  m <- move(visits(finches)) %>%
    ...
    dplyr::summarize(
      path_use = length(.data$move_path) / .data$logger_n[1],
      .groups = "drop"
    )
...
test_that("add_locs handles different data types", {
  ...
  expect_message(
    a <- add_locs(p_none, l_miss),
    "Removed logger 2100 due to missing lat or lon."
  )
  expect_equal(dplyr::filter(p, logger_id != 2100), a, ignore_attr = TRUE)

  expect_message(
    a <- add_locs(m_none, l_miss),
    "Removed logger 2100 due to missing lat or lon."
  )
  expect_equal(
    dplyr::filter(m, !(stringr::str_detect(move_path, "(2100)"))),
    a,
    ignore_attr = TRUE
  )

  expect_message(
    a <- add_locs(p_none, l_miss2),
    "Removed loggers 2100, 2400 due to missing lat or lon."
  )
  expect_equal(
    dplyr::filter(p, !(logger_id %in% c(2100, 2400))),
    a,
    ignore_attr = TRUE
  )

  expect_message(
    a <- add_locs(m_none, l_miss2),
    "Removed loggers 2100, 2400 due to missing lat or lon."
  )

  expect_equal(
    dplyr::filter(m, !(stringr::str_detect(move_path, "(2100)|(2400)"))),
    a,
    ignore_attr = TRUE
  )
})
