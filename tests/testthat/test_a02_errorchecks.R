...
test_that("check_time returns error if col not time", {
  d <- data.frame(
    animal_id = NA,
    logger_id = NA,
    start = as.POSIXct("2015-01-01 12:34:00"),
    end = 54
  )
  ...
  expect_error(
    check_time(d, c("start", "end")),
    "Columns 'start', 'end' must be in R's date/time formating \\(POSIXct\\)."
  )
  expect_error(
    check_time(d, c("start", "end"), internal = FALSE),
    "Consider using as.POSIXct()"
  )
})
...
test_that("check_tz returns UTC if problems", {
  ...
  expect_message(
    expect_equal(check_tz("AMerica/Vancouver"), "America/Vancouver"),
    "not in OlsonNames\\(\\), assuming"
  )
  expect_message(
    expect_equal(check_tz("Amer"), "UTC"),
    "not in OlsonNames\\(\\), defaulting to UTC"
  )

  expect_message(
    expect_equal(check_tz(NULL), "UTC"),
    "Cannot set timezone, defaulting to UTC"
  )
  expect_message(
    expect_equal(check_tz(NA), "UTC"),
    "Cannot set timezone, defaulting to UTC"
  )
  expect_message(
    expect_equal(check_tz(""), "UTC"),
    "Cannot set timezone, defaulting to UTC"
  )
})
...
test_that("check_indiv returns error if more than one animal_id", {
  ...
  expect_silent(check_indiv(data.frame(
    animal_id = rep("041868D396", 3),
    logger_id = 45
  )))
})
...
test_that("check_input renames columns (lat/lon)", {
  ...
  for (i in c("longitude", "LonGiTude", "long", "Long")) {
    ...
    expect_message(
      r2 <- check_input(r),
      paste0("Renaming column '", i, "' to 'lon'")
    )
  ...
  for (i in c("latitude", "LatItuDE")) {
    ...
    expect_message(
      r2 <- check_input(r, input = "lat", options = c("lat", "latitude")),
...
test_that("check_input renames columns (regular)", {
  ...
  expect_message(
    r3 <- check_input(r2, input = "time", options = "time"),
...
test_that("check_input returns error if ambiguous", {
  ...
  expect_error(
    check_input(r, input = "lat", options = c("lat", "latitude")),
