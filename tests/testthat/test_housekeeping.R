...
test_that("check.ids catches and removes correctly", {
  ...
  expect_is(check.ids(r, b)$feeder_id, "factor")
  expect_is(check.ids(r, b)$time, "POSIXct")

  r$bird_id <- as.character(r$bird_id)
  r$bird_id[1] <- substr(r$bird_id[1], 1, 4)
  expect_error(check.ids(r, b), "You have some bird_ids in your read data that are not 10 characters long")
})
