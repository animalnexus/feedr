...
test_that("load_format() loads and formats data correctly", {
  ...
  expect_equal(
    r$time[1],
    as.POSIXct("2016-01-11 10:48:49", tz = current_tz_nodst)
  )
  ...
  # Current tz
  expect_equal(
    load_format(f)$time[1],
    as.POSIXct("2016-01-11 10:48:49", tz = current_tz_nodst)
  )
  expect_equal(
    load_format(f, dst = TRUE)$time[1],
    as.POSIXct("2016-01-11 10:48:49", tz = current_tz)
  )

  # No dst
  expect_equal(
    load_format(f, tz = "America/Vancouver")$time[1],
    as.POSIXct("2016-01-11 10:48:49", tz = "Etc/GMT+8")
  )
  expect_equal(
    load_format(f, tz = "America/Vancouver", tz_disp = "America/Toronto")$time[
      1
    ],
  ...
  # With dst
  expect_equal(
    load_format(f, tz = "America/Vancouver", dst = TRUE)$time[1],
...
test_that("load_format fixes names", {
  ...
  expect_named(
    suppressMessages(load_format(r1)),
    c(
      "animal_id",
      "date",
      "time",
...
test_that("load_raw loads and formats data correctly - logger id from file name", {
  ...
  expect_equal(
    ...
    expect_message()
  expect_equal(
    load_raw(f, tz = "America/Vancouver")$time[2],
    as.POSIXct("2016-01-11 10:48:50", tz = "Etc/GMT+8")
  ) %>%
    expect_message()
  expect_equal(
    load_raw(f, tz = "America/Vancouver", tz_disp = "America/Toronto")$time[2],
  ...
  # With dst
  expect_equal(
    load_raw(f, dst = TRUE)$time[2],
    as.POSIXct("2016-01-11 10:48:50", tz = check_tz(Sys.timezone()))
  ) %>%
    expect_message()
  expect_equal(
    load_raw(f, dst = TRUE, tz = "America/Vancouver")$time[2],
    as.POSIXct("2016-01-11 10:48:50", tz = "America/Vancouver")
  ) %>%
    expect_message()
  expect_equal(
    load_raw(
    ...
    as.POSIXct("2016-01-11 13:48:50", tz = "America/Toronto")
  ) %>%
    expect_message()
})
...
test_that("load_raw loads and formats data correctly - logger id from first line", {
  ...
  expect_s3_class(
    r <- load_raw(r_file = f, details = 1, tz = "America/Vancouver"),
    "data.frame"
  ) %>%
    expect_message()
  expect_s3_class(
...
test_that("load_raw stops/warns if can't get logger_id", {
  ...
  f <- system.file(
    "extdata",
    "import_tests",
    "logger_inname_GR10DATA.TXT",
    package = "feedr"
  )
  expect_warning(
    r <- load_raw(r_file = f, details = 1),
  ...
  expect_equal(
    r$logger_id[1],
    "06200004BF 15/01/2016 10:48:49",
    ignore_attr = TRUE
  )
  expect_warning(
    r <- load_raw(r_file = f, details = 1, verbose = FALSE),
    "logger_id extracted from first line of the file as "
  ) %>%
    expect_warning("All formats failed to parse") %>%
    expect_silent()
  ...
  expect_error(
    r <- load_raw(r_file = f, details = 2),
    "Expecting one pair of lat/lon on second line of the file"
  ) %>%
    expect_message("Loading file")

  expect_error(
    r <- load_raw(r_file = f, details = 2, verbose = FALSE),
    "Expecting one pair of lat/lon on second line of the file"
  ) %>%
    expect_silent()
})
...
test_that("load_raw_all loads and formats data correctly", {
  ...
  expect_equal(
    ...
    factor(
      "GR10DATA",
      levels = c("GR10DATA", "GR11DATA", "GR12DATA", "GR13DATA")
    )
  )
  expect_equal(
    load$time[1],
