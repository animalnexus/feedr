...
types <- list(
  "Logical" = c("logical", "logic", "boolean", "bool"),
...
types <- lapply(types, FUN = function(x) {
...
get_class <- function(x) {
  dots <- sapply(types, function(x) {
    return(formula(paste0("~stringr::str_detect(tolower(desc), '", x, "')")))
  })
...
get_defaults <- function(x) {
  ...
  tibble::tibble(arg = names(f), value = as.character(f)) %>%
    dplyr::filter(value != "")
}
...
get_manual_single <- function(func) {
  ...
  if (any(tags == "\\arguments")) {
    ...
    d <- tibble::tibble(
      arg = unlist(sapply(args, get_argument)),
...
labs <- data.frame(
  rbind(
    ...
    cbind(
      ...
      lab = c(
        "Allow impossible visits?",
        "Remove data with missing values",
        "Min seconds between visits",
        "Min seconds to travel between loggers"
      )
    )
  ),
...
if (nrow(man) > 0) {
  ...
  stop("Manual entries didn't compile")
}
