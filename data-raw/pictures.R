...
pics <- data.frame(
  img = paste0(
    "http://gaia.tru.ca/birdMOVES/img.kl/",
    list.files("./data-raw/Pictures - Formatted/")
  )
) %>%
  dplyr::mutate(
    animal_id_orig = gsub(".*/([^/]+).jpg$", "\\1", img),
...
dbGetQuery(
  con,
  statement = paste0(
    "SELECT birds.bird_id, species.engl_name FROM birds, species WHERE (species.code = birds.species)"
  )
) %>%
  load_format(tz = "") %>%
  dplyr::mutate(
...
file.rename(
  paste0("./data-raw/Pictures - Formatted/", pics$animal_id_orig, ".jpg"),
