...
Our original analysis doesn't have any reversals, meaning that the matrix of interactions reflects a linear hierarchy (or individuals are tied). However, a look at dominance in our chickadee data set shows many reversals. The `reversals` item is a list containing a data frame for each matrix showing which individuals experienced reversals with whom: Pairs of individuals which could be switched either way in the hierarchy.
...
Sometimes you may want to perform the same data transformations multiple times, perhaps for different sites, time periods, or experiments. You could repeat yourself, but the easiest thing to do is to use the `do()` function from the `dplyr` package.

This function allows you to apply functions to groups in your data frame. Use "." to define where the passed on data should go (in our case, this refers to the grouped data frame):

In this example we have two experiments, so we want to calculate things separately:
...
m_avg <- m_exp %>%
  group_by(experiment, animal_id, move_path, logger_id, lat, lon) %>%
