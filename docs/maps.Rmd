...
Sometimes you may want to perform the same summary multiple times, perhaps for different sites, time periods, or experiments. You could repeat yourself, but the easiest thing to do is to use the `dplyr` `do()` function.
...
To plot by individual, simply use individually summarized data sets:
...
```{r}
map_ggmap(p = p_pop, m = m_pop, zoom = 16)
```
...
```{r}
map_ggmap(p = p_pop, m = m_pop, maptype = "terrain")
```

```{r}
map_ggmap(p = p_pop, m = m_pop, maptype = "roadmap")
```
...
Or all in two batches

```{r, fig.width = 10, fig.height = 10}

ids <- unique(p_indiv$animal_id) # Get a vector of animal_ids

# Use only first 10 ids
map_ggmap(p = p_indiv, m = m_indiv,
          which = ids[1:10])
```

Note that several individuals (041869123C, 062000043E, 0620000513) were removed. If we take a closer look at those individuals  in the data frame, we see that they had no movements:

```{r}
filter(p_indiv, animal_id %in% c("041869123C", "062000043E", "0620000513"))
filter(m_indiv, animal_id %in% c("041869123C", "062000043E", "0620000513"))
```
