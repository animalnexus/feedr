...
```{r echo = FALSE}
all.files <- list.files(system.file("extdata", "raw", "exp2", package = "feedr"), full.names = T)
...
```{r echo = FALSE}
r <- load_raw_all(r_dir = list.files(system.file("extdata", package = "feedr"), pattern = "raw", full.names = T), extra_pattern = "exp[1-9]{1}", extra_name = "experiment")
head(r)
```
