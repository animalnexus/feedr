  ...
  if (!all(sapply(d[, n], class) == c("POSIXct", "POSIXt"))) {
    stop(
      ...
      ifelse(
        internal == FALSE,
        paste0(
          " Consider using as.POSIXct() and strptime() or ",
...
check_indiv <- function(d) {
  if (length(unique(d$animal_id)) > 1) {
    stop(
      "This function is only designed to be run on one individual at a time.",
      "Consider using the nest() from tidyr and map() from purrr ",
      "to apply this function to all animals.",
      call. = FALSE
    )
  }
}
...
check_format <- function(d, map = FALSE, disp = FALSE) {
  ...
  if (!map) {
    msg_l <- paste0(
      msg_l,
      " You should remove any '_'s if you plan to use these functions."
    )
  }
  if (!disp) {
    msg_a <- paste0(
  ...
  if ("logger_id" %in% names(d)) {
    if (any(stringr::str_count(d$logger_id, "_") > 0)) message(msg_l)
  }
  if ("animal_id" %in% names(d)) {
    if (any(stringr::str_count(d$animal_id, "_") > 0)) message(msg_a)
  }
}
...
check_input <- function(
  ...
  n <- which(stringr::str_detect(
    names(d),
    stringr::regex(opts_string, ignore_case = TRUE)
  ))
  ...
  if (length(n) > 0) {
    ...
    if (length(n) > 1) {
      ...
      if (verbose) {
        message("Omitting duplicate columns for ", input)
      }
      d <- d[, -n[2:length(n)]]
      n <- n[1]
    }
    if (any(names(d)[n] != input)) {
      if (verbose) {
        message("Renaming column '", names(d)[n], "' to '", input, "'")
      }
      names(d)[n] <- input
    }
  }
