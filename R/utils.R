...
tz_offset <- function(tz, dst = FALSE, tz_name = FALSE) {
  ...
  if (dst) {
    t <- as.numeric(difftime(
      as.POSIXct("2016-06-01 00:00:00", tz = "UTC"),
      as.POSIXct("2016-06-01 00:00:00", tz = tz),
      units = "hours"
    ))
  }
  if (tz_name) {
...
mp <- function(x) {
  paste0(sort(unlist(strsplit(as.character(x), "_"))), collapse = "_")
}
...
keep_extra <- function(d, n, only = c("animal_id", "logger_id", "date")) {
  ...
  if ("animal_id" %in% only) {
    animal_id <- extra[
      lapply(extra, FUN = function(x) {
        nrow(unique(cbind(d$animal_id, d[, x])))
      }) ==
        length(unique(d$animal_id))
    ]
  }
  if ("logger_id" %in% only) {
    logger_id <- extra[
      lapply(extra, FUN = function(x) {
        nrow(unique(cbind(d$logger_id, d[, x])))
      }) ==
        length(unique(d$logger_id))
    ]
  }
  if ("date" %in% only) {
    date <- extra[
      lapply(extra, FUN = function(x) nrow(unique(cbind(d$date, d[, x])))) ==
  ...
  if (length(all) > 0) {
    all <- unique(d[, c("animal_id", "logger_id", "date", all)])
  } else {
    all <- NULL
  }
  if (length(animal_id) > 0) {
    animal_id <- unique(d[, c("animal_id", animal_id)])
  } else {
    animal_id <- NULL
  }
  if (length(logger_id) > 0) {
    logger_id <- unique(d[, c("logger_id", logger_id)])
  } else {
    logger_id <- NULL
  }
  if (length(date) > 0) {
    date <- unique(d[, c("date", date)])
  } else {
    date <- NULL
  }
  return(list(
...
merge_extra <- function(d, extra, only = NULL) {
  if (!is.null(extra$all)) {
    d <- dplyr::left_join(
      d,
      extra$all,
      by = c("animal_id", "logger_id", "date")
    )
  }
  if (!is.null(extra$animal_id)) {
    d <- dplyr::left_join(d, extra$animal_id, by = "animal_id")
  }
  if (!is.null(extra$logger_id)) {
    d <- dplyr::left_join(d, extra$logger_id, by = "logger_id")
  }
  if (!is.null(extra$date)) {
    d <- dplyr::left_join(d, extra$date, by = "date")
  }
  return(d)
}
...
mean_clock <- function(time, origin = FALSE) {
  ...
  mean_time <- format(
    mean(as.POSIXct(paste("1970-01-01", format(time, "%H:%M:%S")))),
    "%H:%M:%S"
  )
  mean_date <- ifelse(
    origin,
    "1970-01-01",
    as.character(lubridate::as_date(mean(time)))
  )
...
remove_edges <- function(x, dir_to, dir_from) {
  if (x$direction[1] == dir_to) {
    x <- x[-1, ]
  }
  if (nrow(x) > 0) {
    if (x$direction[nrow(x)] == dir_from) x <- x[-nrow(x), ]
  }
  x
}
