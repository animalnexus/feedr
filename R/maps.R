...
#'   the number of individuals in the data, "indiv" calculates totals for each
#'   individual.
#' @param p_scale,m_scale Numerical. Relative scaling constants to increase (>
#'   1) or decrease (< 1) the relative size of presence (p) and path (m) data.
#' @param p_title,m_title Character. Titles for the legends of presence (p) and
#'   path (m) data.
#' @param p_pal,m_pal Character vectors. Colours used to construct gradients for
#'   presence (p) and movement (m) data.
#' @param maptype Character. The type of map to download. See `maptype`
#'   under [ggmap::get_map()] for more options.
#' @param mapsource Character. The source of the map to download. See
#'   `source` under [ggmap::get_map()] for more options.
#' @param zoom Numeric. The zoom level of the map to download. See `zoom`
#'   under [ggmap::get_map()] for more options.
#' @param which Character vector. A vector of animal ids specifying which to
#'   show. Only applies when using individual data.
#' @param u.scale,p.scale,u.title,p.title,u.pal,p.pal Depreciated.
#'
...
map_ggmap <- function(p = NULL, m = NULL, locs = NULL,
  ...
  stop("map_ggmap is defunct for now", call. = FALSE)
  ...
  if(any(names(p) == "animal_id", names(m) == "animal_id")) {
    if(is.null(which)) {
      which <- as.character(unique(c(as.character(p$animal_id),
                                     as.character(m$animal_id))))
    }

    animal_id <- unique(unlist(list(p$animal_id, m$animal_id)))

    if(length(animal_id) > 10 & length(which) > 10) {
      stop("You have chosen to run this function on more than 10 animals. ",
           "This may overload your system. We recommend trying again using ",
           "the 'which' argument to specify a subset of animals.")
    }
    p <- droplevels(p[p$animal_id %in% which,])
    m <- droplevels(m[m$animal_id %in% which,])

    temp_p <- dplyr::group_by(p, animal_id) %>%
      dplyr::summarize(sum = sum(amount))
    temp_m <- dplyr::group_by(m, animal_id) %>%
      dplyr::summarize(sum = length(path_use))

    keep_id <- intersect(temp_p$animal_id[temp_p$sum > 0],
                         temp_m$animal_id[temp_m$sum > 0])
    if(length(setdiff(which, keep_id)) > 0) {
      message("Some animal_ids removed due to lack of data: ",
              paste(setdiff(which, keep_id), collapse = ", "))
  ...
  #  # Sort and Scale
    ...
    m$path_use2 <- scale_area(m$path_use, max = 3 * m_scale,
                              min = 0.1 * m_scale)
  }
  ...
  # Add logger points
  map <- map + ggplot2::geom_point(data = locs, ggplot2::aes(x = lon, y = lat),
                                   colour = "black")

  # If Multiple animal ids included
  if(!is.null(animal_id)) {
    map <- map + ggplot2::facet_wrap(~ animal_id)
    message("You have specified multiple animals and static maps, ",
            "this means that an individual map will be drawn for each animal. ",
            "This may take some time to display in the plots window.")
  }
  map
}
