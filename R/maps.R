...
map_prep <- function(p = NULL, m = NULL, locs = NULL, summary = "none") {
  ...
  if (!is.null(m) && nrow(m) > 0) {
    ...
    if (!all(c("move_path", "logger_id") %in% names(m))) {
      msg <- c(
        msg,
        "The movement dataframe (m) requires two columns: ",
        "'logger_id' and 'move_path'"
      )
    }

    if (
      "move_path" %in% names(m) && any(stringr::str_count(m$move_path, "_") > 1)
    ) {
      msg <- c(
        msg,
        "Using '_' in logger_id names conflicts with the ",
  ...
  if (nrow(locs) == 0) {
    stop("Data is missing latitude (lat) ", "or longitude (lon) or both.")
  }
  ...
  if (summary != "none") {
    ...
    if (xor("animal_id" %in% names(p), "animal_id" %in% names(m))) {
      msg <- c(
        msg,
        "Movements (m) and presence (p) data do not both have an ",
      ...
      if ("animal_id" %in% names(p)) {
        ...
        if (nrow(unique(m[, c("move_path", "animal_id")])) != nrow(m) / 2) {
          msg <- c(
            msg,
            "Movement data should be summarized to have two rows ",
        ...
        if (
          nrow(unique(m[, c("move_path", "path_use")])) !=
            length(unique(m$move_path))
        ) {
          msg <- c(
            msg,
            "Movement data should have identical path_use values ",
  ...
  l <- leaflet::leaflet(
    ...
    leaflet::addCircleMarkers(
      ...
      popup = htmltools::htmlEscape(
        paste("Logger:", as.character(unlist(locs[, marker])))
      ),
      group = "Loggers",
      weight = 2,
      opacity = 1,
...
path_lines <- function(
  ...
  leaflet::addPolylines(
    ...
    group = m_title,
    popup = ~ as.character(round(
      path_use[1],
      digits = if (max(path_use[1]) < 10) 1 else 0
    )),
    layerId = layerId
  )
}
...
path_layer <- function(
  ...
  if (!missing(p.scale)) {
    warning(
      "Argument p.scale is deprecated; please use m_scale instead.",
      call. = FALSE
    )
    m_scale <- p.scale
  }
  ...
  if (!missing(p.title)) {
    warning(
      "Argument p.title is deprecated; please use m_title instead.",
      call. = FALSE
    )
    m_title <- p.title
  }
  ...
  if (!missing(p.pal)) {
    warning(
      "Argument p.pal is deprecated; please use m_pal instead.",
      call. = FALSE
    )
    m_pal <- p.pal
  }
  ...
  m_pal <- leaflet::colorNumeric(
    palette = grDevices::colorRampPalette(m_pal)(15),
    domain = m$path_use
  )
  ...
  for (path in unique(m$move_path)) {
    map <- path_lines(
      ...
      m_title = m_title,
      m_pal = m_pal,
      val_max = max(m$path_use),
      val_min = min(m$path_use),
      layerId = path
    )
  }
...
presence_layer <- function(
  ...
  if (!missing(u.scale)) {
    warning(
      "Argument u.scale is deprecated; please use p_scale instead.",
      call. = FALSE
    )
    p_scale <- u.scale
  }
  ...
  if (!missing(u.title)) {
    warning(
      "Argument u.title is deprecated; please use p_title instead.",
      call. = FALSE
    )
    p_title <- u.title
  }
  ...
  if (!missing(u.pal)) {
    warning(
      "Argument u.pal is deprecated; please use p_pal instead.",
      call. = FALSE
    )
    p_pal <- u.pal
  }
  ...
  # Define palette
  p_pal <- colorNumeric(
    palette = grDevices::colorRampPalette(p_pal)(15),
    domain = p$amount
  )
