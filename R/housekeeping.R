...
check_ids <- function(
  ...
  if (!missing(bird_ids)) {
    warning(
      "Argument bird_ids is deprecated; please use ids instead.",
      call. = FALSE
    )
    ids <- bird_ids
  }
  ...
  if (is.null(ids) || (length(ids) > 1 & !is.data.frame(ids))) {
    stop(
      "animal_id index (id) should either be the name of a comma separated ",
  ...
  if (!is.data.frame(ids)) {
    ids <- utils::read.csv(ids)
  }

  # Check for required columns in ids
  if (!all(c("animal_id", "species") %in% names(ids))) {
    stop(
      "animal_id index (id) should contain at least two columns: ",
      "'animal_id' and 'species'",
      call. = FALSE
    )
  }
  ...
  if (!is.na(id_length)) {
    ...
    if (any(nchar(as.character(ids$animal_id)) != id_length)) {
      msg <- c(
        msg,
        paste0(
          "You have some ids in your animal_id index that are not ",
          id_length,
          " characters long"
        )
      )
    ...
    if (any(nchar(as.character(r$animal_id)) != id_length)) {
      msg <- c(
        msg,
        paste0(
          "You have some ids in your read data that are not ",
          id_length,
          " characters long"
        )
      )
  ...
  if (length(unlisted) > 0) {
    message(
      "Some animal_ids present in your data do not exist in the ",
      "animal_id index: ",
      paste(unlisted, collapse = ", ")
    )
  } else {
    message("All animal_ids in your data are also in your animal_id index")
  }
  ...
  if (length(unseen) > 0) {
    message(
      "Some animal_ids present in your animal_id index, are not in your data: ",
      paste0(unseen, collapse = ", ")
    )
  } else {
    message("All animal_ids in your animal_id index are also in your data")
  }
  ...
  if (length(ob2) > 0) {
    message(
      "The following animal_ids have been omitted: ",
      paste0(ob2, collapse = ", ")
    )
  } else {
    message("No animal_ids have been omitted")
  }
...
check_problems <- function(r, problems) {
  if (length(problems) > 1 & !is.data.frame(problems)) {
    stop(
      "Problems should either be the name of a comma separated file (csv) ",
      "OR should be a data frame. In either case, the data should contain ",
      "headers 'original_id' and 'corrected_id'",
      call. = FALSE
    )
  }
  ...
  if (!is.data.frame(problems)) {
    problems <- utils::read.csv(problems)
  }

  if (!all(names(problems) %in% c("original_id", "corrected_id"))) {
    stop(
      "Problems should contain headers 'original_id' and 'corrected_id'",
      call. = FALSE
    )
  }

  if (nrow(problems) < 1) {
    stop("Problems data frame has no rows", call. = FALSE)
  }
  ...
  for (i in 1:nrow(problems)) {
    ...
    r$animal_id[
      r$animal_id == problems$original_id[i]
    ] <- problems$corrected_id[i]
  }
  ...
  if (nrow(fixes) > 0) {
    ...
    message("No animal ids needed to be fixed")
  }
