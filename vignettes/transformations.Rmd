...
map1[label = 'map.ggmap()', URL = '#mapggmap', tooltip = 'Visualize with a static map']
map2[label = 'map.leaflet()', URL = '#mapleaflet', tooltip = 'Visualize with an interactive map']
...
```{r eval = FALSE}
v <- visits(finches, allow.imp = TRUE, na.rm = TRUE)
```
...
Here we've told ddply to split the data frame `v` by `bird_id`, and then to create 4 new columns (`n.feeders`, `n.visits`, `n.days`, `mean.visit.length`). In each case, we have defined how we want ddply to fill these new columns.

```{r}
library(plyr)
s <- ddply(v, c("bird_id"), summarise,
           n.feeders = length(unique(feeder_id)),
           n.visits = length(start),
           n.days = length(unique(day)),
           mean.visit.length = mean(end - start))
s
```
...
library(ggplot2)
ggplot(data = s, aes(x = bird_id, y = mean.visit.length)) +
...
```{r}
r <- get.data(start = "2016-03-07", end = "2016-03-11")
v <- visits(r)
d <- disp(v)
class(d)
names(d)
...
```{r}
r <- get.data(start = "2016-03-07", end = "2016-03-31")
...
- `feeder_left` id of the originating feeder
- `feeder_arrived` id of the destination feeder
...
```{r}
s <- ddply(m, c("move_path"), summarize,
           n = length(move_path))
s
```
...
```{r}
s <- ddply(m, c("move_path", "move_dir"), summarize,
           n = length(move_path))
s
```
...
```{r}
s <- ddply(m, c("bird_id"), summarize,
           n = length(move_path[!is.na(move_path)]))
s
```

Note that we have a zero for 062000043E because we made sure to include all individuals in the analysis (all = TRUE, above) and because we specified ddply to count only non-NA paths (`is.na(move_path)`).
