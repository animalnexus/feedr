...
Note that for several examples of summarizing, we'll be using functions pipes `|>`, which essentially 'pipes' the output of the first line to the next line, see this explanation of [using the pipe in data transformations](https://r4ds.hadley.nz/data-transform.html#sec-the-pipe) for more details.
...
In all cases you'll want to use the `visits()` function first, but after that it depends on what kind of data you want to end up with.
...
```{r}
finches_lg[finches_lg$animal_id == "06200004F8", ]
v[v$animal_id == "06200004F8", ]
```
...
```{r}
n_distinct(v$animal_id)
n_distinct(v$logger_id)
```
...
nlevels(m$animal_id) # The number of levels of animal_id
```
...
```{r}
move_summary <- summarize(
  m,
  n = length(move_path),
  .by = c("move_path", "logger_id")
)
move_summary
```
...
move_summary <- summarize(
  m,
  n = length(move_path),
  .by = c("logger_id", "move_path", "move_dir")
)
move_summary
```
...
move_summary <- summarize(
  m,
  n = length(move_path[!is.na(move_path)]), # Number of movements omitting NAs
  .by = "animal_id"
)
move_summary
```
...
presence_summary <- summarize(
  p,
  n = length(start), # how many rows = number of events
...
ggplot(
  ...
  aes(x = logger_id, y = mean_length, fill = logger_id)
) +
  geom_bar(stat = "identity") +
  geom_errorbar(
...
full_summary <- full_join(disp_summary, presence_summary, by = "logger_id") |>
  mutate(
    n = replace(n, is.na(n), 0), # First replace the missing values of n with 0
    n_per_hour = n / amount * 60
  ) # Calculate the number of interactions as an hourly rate of hours actually used

full_summary

ggplot(data = full_summary, aes(x = logger_id, y = n_per_hour)) +
  geom_bar(stat = "identity")
```
...
--------------------------
Go back to [housekeeping](housekeeping.html) | Continue with [visualizations](maps.html)
