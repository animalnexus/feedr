...
- [`load.raw()`](#loadraw)
- [`load.raw.web()`](#loadrawall)
- [`load.web()`](#loadweb)
- [`load.web.legacy()`](#loadweblegacy)
- [`get.data()`](#getdata)
...
`load.web.legacy()` is simply a function for loading in web data that is in the old format (if you don't understand, ignore this).
...
r1 <- load.raw(all.files[1])
```

```{r eval = FALSE}
r1 <- load.raw("./Data/exp1/GR10DATA_2015_12_09.TXT")
head(r1)
```
...
```{r eval = FALSE}
r1 <- load.raw("./Data/exp1/Feeder_10_2015_12_09.TXT", feeder_pattern = "Feeder_[0-9]{2}")
head(r1)
```
...
The function `load.raw.all()` is a wrapper function which will automatically load and combine data contained in several different files in a single folder, or in a nested series of folders. Other files can be present, but all feeder data files must be identifiable by a pattern in the file name.
...
```{r eval = FALSE}
r <- load.raw.all(r_dir = "./Data/raw")
head(r)
```

```{r echo = FALSE}
r <- load.raw.all(r_dir = list.files(system.file("extdata", package = "feedr"), pattern = "raw", full.names = T))
head(r)
```

If your feeder files didn't have a identiifable label, but were the only csv files in the folders, you could use:
```{r eval = FALSE}
r <- load.raw.all(r_dir = "./Data/raw", pattern = ".csv")
```
...
```{r eval = FALSE}
r <- load.raw.all(r_dir = "./Data/raw", extra_pattern = "exp[1-2]{1}", extra_name = "experiment")
```

```{r echo = FALSE}
r <- load.raw.all(r_dir = list.files(system.file("extdata", package = "feedr"), pattern = "raw", full.names = T), extra_pattern = "exp[1-2]{1}", extra_name = "experiment")
head(r)
```
...
```{r, echo = FALSE}
r <- load.web(system.file("extdata", "load_web.csv", package = "feedr"))
r.old <- load.web.legacy(system.file("extdata", "load_web_legacy.csv", package = "feedr"))
```
...
<a id="loadweblegacy"></a>
If you have an old csv file from the original website, you can still load this in with:
```{r, eval = FALSE}
r.old <- load.web.legacy("./Data/load_web_legacy.csv")
head(r.old)
```
```{r, echo = FALSE}
head(r.old)
...
In either case, the default timezone is "Canada/Pacific", therefore, if your data is from a different time zone, you need to specify that manually. Make sure you use a timezone name from the output of OlsonNames(). For example:

```{r, eval = FALSE}
r <- load.web("./Data/load_web.csv", tz = "America/Costa_Rica")
r$time[1]
head(r)
```

```{r, echo = FALSE}
r <- load.web(system.file("extdata", "load_web.csv", package = "feedr"), tz = "America/Costa_Rica")
r$time[1]
head(r)
```
...
If you don't specify anything, all data will be downloaded for the default site (Kamloops) and the default extra columns (loc and species) with the default timezone (America/Vancouver). See ?get.data for more details.

```{r, echo = FALSE}
r <- get.data(end = "2015-09-06")
head(r)
```
...
```{r}
r <- get.data(start = "2015-10-01", end = "2015-10-02")
head(r)
```

```{r}
r <- get.data(start = "2015-10-01 09:00:00", end = "2015-10-02")
head(r)
```
...
Extra bird or feeder related details (?get.data for more):

```{r}
r <- get.data(end = "2015-09-06", feeder_details = c("loc","site_name"), bird_details = c("species", "age", "sex"))
head(r)
```

Different Sites:
```{r}
r <- get.data(end = "2013-02-14 09:00:00", site = "Costa Rica", feeder_details = c("loc","site_name"))
head(r)
```
--------------------------
Back to [top](#top)  
