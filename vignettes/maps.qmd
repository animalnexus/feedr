...
map_leaflet(
  ...
  p_scale = 0.5,
  m_scale = 0.5,
  p_title = "Presence (min)",
  m_title = "Paths (Total use)",
  p_pal = c("blue", "white"),
  m_pal = c("black", "red")
)
```
...
```{r}
p_pop <- summarize(p, amount = sum(length) / animal_n[1], .by = "logger_id")
p_pop
```
...
m_pop <- summarize(
  m,
  path_use = length(move_path) / animal_n[1],
  .by = c("logger_id", "move_path", "lat", "lon")
)
m_pop
```
...
```{r}
p_indiv <- summarize(p, amount = sum(length), .by = c("animal_id", "logger_id"))
p_indiv

m_indiv <- summarize(
  m,
  path_use = length(move_path),
  .by = c("animal_id", "move_path", "logger_id", "lat", "lon")
)
m_indiv
```
...
> Note: While feedrUI still generally works, it is a lower maintenance priority so features many not be stable.
...
--------------------------
Go back to [transformations](transformations.html)
