...
And make sure we have the data files we need: 

```{r}
# Create the Data folder in your working directory
dir.create("Data")

# Copy the animal index file to your Data folder
file.copy(
  system.file(
    "extdata",
    c("animal_index.csv", "problems.csv"),
    package = "feedr"
  ),
  "Data"
)
...
This tutorial assumes that you already know how to [load/import](load.html) your data. We will be using the feedr package example data set: finches (loaded when you load the feedr package with `library(feedr)`).
```{r}
finches
```
...
```{r}
animal_index <- read_csv("Data/animal_index.csv")
animal_index
```

`check_ids()` works by comparing the index to a feedr data set, and then returns a cleaned data set. Here we'll save it as `r_clean`:
```{r}
...
__Note__: You can also skip reading the index and simply provide `check_ids()` with the location of the index file:
...
```{r}
filter(r_clean, animal_id %in% c("0620000514", "041868D861"))
```
...
This function will correct all instances of an `animal_id` according to the list you provide.

```{r}
problems <- read_csv("Data/problems.csv")
problems
```
...
--------------------------
Go back to [loading data](load.html) | Continue with [transformations](transform.html)
